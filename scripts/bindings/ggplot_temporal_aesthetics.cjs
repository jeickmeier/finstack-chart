...
for(const [index,testcase] of cases.entries())for(const [unit,multiplier] of [['s',1n],['ms',1000n],['us',1000000n],['ns',1000000000n]]){
 ...
 try{
  ...
  const wire=plot.to_json();assert.equal(JSON.parse(wire).version,['colour','fill'].includes(channel)?42:45);const restored=c.Plot.from_json(wire);owned.push(restored);assert.equal(restored.to_json(),wire);
  let baseEncoding;
  for(const state of ['original','edited']){
   ...
   styles.forEach((style,i)=>{const value=wanted[i];if(channel==='alpha'){assert.equal(style.color.alpha,value===null?255:alphaByte(value));}else if(target){const actual=style[{size:'radius',alpha:'alpha',linewidth:'stroke_width'}[channel]]??null;assert.equal(actual===null,value===null);if(value!==null)assert.ok(Math.abs(actual-value)<2e-12);}else{const paint=style[channel==='fill'?'fill':'color'],rgb=value==='grey50'?[127,127,127]:[1,3,5].map(i=>parseInt(value.slice(i,i+2),16));assert.deepEqual([paint.red,paint.green,paint.blue],rgb);assert.equal(paint.alpha,255);}});
