"""FIX-S05/09 independent host scenes and publication artifacts at both resolutions."""
...
def compare(actual,expected,path='',errors=None):
    ...
    elif type(actual) in (int,float) and type(expected) in (int,float) and re.fullmatch(r'(?:/items/\d+/primitive/(?:ShapePath|VectorPath)/geometry/commands/\d+/(?:MoveTo/\d|LineTo/\d|QuadraticTo/\d|CubicTo/\d|Arc/(?:radius|to/\d))|/items/\d+/primitive/ShapePath/anchors/\d+/(?:x|y))',path):
...
(root/'comparison.json').write_text(json.dumps(records,indent=2)+'\n');print('PASS radial/link comparison: three hosts, three themes, two resolutions, complete scene metadata and exact PNG/PDF bytes; SVG equality and any coordinate differences recorded. Visual/native inspection is separate.')
