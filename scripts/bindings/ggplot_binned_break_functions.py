"""FIX-GG04: binned break functions through the actual Python adapter."""
...
constructor_guides="--constructor-guides" in sys.argv[3:]
cases=json.loads((ROOT/('fixtures/parity/ggplot2/binned-break-default-names.json' if default_names else 'fixtures/parity/ggplot2/binned-break-functions.json')).read_text())['cases']
def descriptor(t):
 ...
 return {'training':'Eligible','function':{'Interpolated':{'normalization':{'Ggplot':{'family':family,'domain':[0,1],'reverse':t['transform']=='reverse','rescaler':'Range'}},'output':output,'unknown':{'kind':'Missing'}}},'ggplot':{'Binned':{'limits':[1,10] if t['limits']=='full' else None,'oob':'Squish','breaks':{'Equal':{'none':5,'three':3,'zero':0}[t['count']]},'right':True}},'breaks_function':{'operation':{'id':'example.breaks_'+t['signature'],'version':'1'},'parameters':t['mode']},'guide':{('BinnedSteps' if t['channel']=='colour' else 'BinnedBins') if constructor_guides else 'Binned':('Automatic' if default_names else {'Registered':{'operation':{'id':'example.scale_labels','version':'1'},'parameters':'indexed'}})}}
def layer(t):
...
def check(t,chart):
 ...
 if t['channel']=='colour':
  entries=(state.get('color_legend') or {}).get('numeric_breaks',[]);actual={'labels':[e['label'] for e in entries if e['visible']]}
  ...
  expected=dict(next(iter(t['result'].get('keys',[])),{'values':[],'labels':[]}));expected['values']=t['result']['boundaries']
  if len(expected['values'])==1:expected['values']=expected['values']*2
...
for index,t in enumerate(cases):
 owned=[]
 try:
  d=data_for(t);owned.append(d);p=build(t,d);owned.append(p);wire=p.to_json();assert json.loads(wire)['version']==(44 if constructor_guides else 33)
  ...
  assert 'error' in t['result'],(t,str(error));assert error.code in ('CHART_NUMERICAL_DOMAIN','CHART_VALIDATION'),(t,error.code)
  records.append({'index':index,'error':error.code})
 finally:
  for obj in reversed(owned):obj.dispose()
assert len(records)==(116 if default_names else 4680),len(records)
for channel in (() if default_names else ('size','colour')):
 selected={t['population']:t for t in cases if t['channel']==channel and t['limits']=='none' and t['transform']=='identity' and t['mode']=='domain' and t['signature']=='n' and t['count']=='three'};owned=[]
 try:
  ...
  for obj in reversed(owned):obj.dispose()
assert len(records)==(116 if default_names else 4686)
(out/'records.json').write_text(json.dumps(records,indent=2));registry.dispose()
print(f'PASS Python: {len(records)} binned break states; default_names={default_names}.')
