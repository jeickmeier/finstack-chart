...
function author(rows,classifier,facets){
 const d=data(rows),scale=new c.StandaloneScale(classifier?'quantile':'ordinal',{range:['red','blue','green']});
 let p=c.plot(d).aes(c.aes().x('value').y('value').color(classifier?'value':'key').color_scale('shared')).layer(c.points())
 .scale(c.color_mapped('shared',scale,'Eligible')).x_axis(c.x_axis().scale(c.scale_linear().domain(0,100))).y_axis(c.y_axis().scale(c.scale_linear().domain(0,100)));
...
for(const classifier of [false,true])for(const facets of [false,true]){
 ...
 records.push({family:classifier?'quantile':'ordinal',facets,updates:4,fresh_batch_rgba_equal:true,retained_capture:true});
}
fs.writeFileSync(out,JSON.stringify(records,null,2)+'\n');console.log('PASS SP-07 WASM: sixteen append/upsert/remove/retention steps match fresh batch PNG; exact uint64 categories and old captures survive.');
