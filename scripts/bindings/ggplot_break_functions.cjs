...
const cases=JSON.parse(fs.readFileSync(path.join(root,defaultNames?'fixtures/parity/ggplot2/numeric-break-default-names.json':'fixtures/parity/ggplot2/numeric-break-functions.json'))).cases;
function descriptor(t){const family=t.transform==='sqrt'?{Pow:{exponent:.5}}:t.transform==='log10'?{Log:{base:10}}:'Linear',output=t.channel==='colour'?{Interpolate:{operation:'GgplotPalette',spec:{Gradient:{colors:[{red:19,green:43,blue:67,alpha:255},{red:86,green:177,blue:247,alpha:255}],values:null}}}}:{Interpolate:{operation:'PowerRange',range:[1,6],exponent:.5,absolute:false}};return {training:'Eligible',function:{Interpolated:{normalization:{Ggplot:{family,domain:[0,1],reverse:t.transform==='reverse',rescaler:'Range'}},output,unknown:{kind:'Missing'}}},ggplot:{Continuous:{limits:t.limits==='full'?[1,10]:null,oob:'Censor'}},breaks_function:{operation:{id:'example.breaks_'+t.signature,version:'1'},parameters:t.mode},guide:{Continuous:{count:{none:null,three:3,zero:0}[t.count],labels:(defaultNames?'Automatic':{Registered:{operation:{id:'example.scale_labels',version:'1'},parameters:'indexed'}})}}};}
...
function check(t,chart){const state=chart.semantics().layers[0];let actual,expected;
 if(t.channel==='colour'){actual={labels:(state.color_legend?.entries??[]).map(e=>e[0])};expected={labels:(t.result.keys??[]).flatMap(g=>g.labels)};}
 else {const entries=Object.values(state.numeric_value_guides??{}).flat().filter(e=>e.visible);actual={values:entries.map(e=>e.transformed),labels:entries.map(e=>e.label)};expected=t.result.keys?.[0]??{values:[],labels:[]};assert.equal(actual.values.length,expected.values.length,JSON.stringify(t));actual.values.forEach((a,i)=>assert.ok(Math.abs(a-expected.values[i])<=3e-12*Math.max(1,Math.abs(expected.values[i])),JSON.stringify(t)));}
...
for(const [index,t] of cases.entries()){
 ...
 }catch(error){assert.ok(error instanceof c.ChartError&&t.result.error,`${index}: ${error.stack}`);assert.equal(error.code,'CHART_NUMERICAL_DOMAIN');records.push({index,error:error.code});}
 finally{for(const obj of owned.reverse())obj.dispose();}
}
assert.equal(records.length,defaultNames?160:5328);
for(const channel of (defaultNames?[]:['size','colour'])){
 const selected=Object.fromEntries(cases.filter(t=>t.channel===channel&&t.limits==='none'&&t.transform==='identity'&&t.mode==='mixed'&&t.signature==='n'&&t.count==='three').map(t=>[t.population,t])),owned=[];
 try{
...
assert.equal(records.length,defaultNames?160:5336);fs.writeFileSync(path.join(out,'records.json'),JSON.stringify(records,null,2));registry.dispose();
console.log(`PASS WASM: ${records.length} numeric break states; defaultNames=${defaultNames}.`);
