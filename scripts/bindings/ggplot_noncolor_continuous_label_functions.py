"""FIX-GG04: non-color continuous guide labels through the actual Python adapter."""
...
cases=[t for t in json.loads((ROOT/'fixtures/parity/ggplot2/noncolor-numeric-label-functions.json').read_text())['cases'] if t['kind']=='continuous']
def descriptor(t):
...
assert len(records)==2484,len(records)
...
print('PASS Python: 2496 non-color continuous label states, including 12 replacements.')
