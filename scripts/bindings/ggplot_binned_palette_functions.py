"""FIX-GG04: binned vector palette values, lookup errors and immutable host publication."""
...
cases=[t for t in json.loads((ROOT/'fixtures/parity/ggplot2/palette-functions.json').read_text())['cases'] if t['family']=='binned']
cases += [t for t in json.loads((ROOT/'fixtures/parity/ggplot2/vector-palette-lookup.json').read_text())['cases'] if t['family']=='binned']
cases += [t for t in json.loads((ROOT/'fixtures/parity/ggplot2/vector-palette-missing-paint.json').read_text())['cases'] if t['family']=='binned']
def descriptor(t):
 return {'missing_paint_is_na':t['channel']=='colour','training':'Eligible','function':{'Interpolated':{'normalization':{'Ggplot':{'family':'Linear','domain':[0,1],'reverse':False,'rescaler':'Range'}},'output':'Identity','unknown':{'kind':'Missing'}}},'ggplot':{'Binned':{'limits':[1,10] if t['limits']=='full' else None,'oob':'Squish','breaks':{'Explicit':[1]} if t.get('guide_mode')=='first' else {'Nice':5},'right':True}},'guide':'Hidden' if t.get('guide_mode')=='hidden' else {'Binned':'Automatic'},'palette_function':{'operation':{'id':t.get('operation','example.scale_palette'),'version':'1'},'parameters':{'mode':t['mode'],'channel':t['channel']}}}
...
def check(t,chart):
 state=chart.semantics()['layers'][0];styles=state.get('styles',[]);wanted=t['result']['mapped']
...
print(f'PASS Python: {len(records)} binned vector palette states including 24 replacements and four rejection contracts.')
