"""SP-07 actual Python scale retraining during keyed updates and retained captures."""
...
def author(rows,classifier,facets):
 d=data(rows);scale=c.StandaloneScale('quantile' if classifier else 'ordinal',range=['red','blue','green'])
 p=c.plot(d).aes(c.aes().x('value').y('value').color('value' if classifier else 'key').color_scale('shared')).layer(c.points())
 p=p.scale(c.color_mapped('shared',scale,'Eligible')).x_axis(c.x_axis().scale(c.scale_linear().domain(0.,100.))).y_axis(c.y_axis().scale(c.scale_linear().domain(0.,100.)))
...
for classifier in [False,True]:
 for facets in [False,True]:
  ...
  records.append({'family':'quantile' if classifier else 'ordinal','facets':facets,'updates':4,'fresh_batch_rgba_equal':True,'retained_capture':True})
out.write_text(json.dumps(records,indent=2)+'\n')
print('PASS SP-07 Python: sixteen append/upsert/remove/retention steps match fresh batch PNG; exact uint64 categories and old captures survive.')
