...
const cases=JSON.parse(fs.readFileSync(path.join(root,'fixtures/parity/ggplot2/noncolor-numeric-label-functions.json'))).cases.filter(t=>t.kind==='continuous');
...
function check(t,chart){const state=chart.semantics().layers[0],entries=Object.values(state.numeric_value_guides??{}).flat().filter(e=>e.visible),actual={values:entries.map(e=>e.transformed),labels:entries.map(e=>e.label)},expected=t.result.keys?.[0]??{values:[],labels:[]};assert.deepEqual(actual.labels,expected.labels,JSON.stringify(t));assert.equal(actual.values.length,expected.values.length,JSON.stringify(t));actual.values.forEach((a,i)=>assert.ok(Math.abs(a-expected.values[i])<=3e-12*Math.max(1,Math.abs(expected.values[i])),JSON.stringify(t)));return actual;}
...
assert.equal(records.length,2484);
...
assert.equal(records.length,2496);fs.writeFileSync(path.join(out,'records.json'),JSON.stringify(records,null,2));registry.dispose();
console.log('PASS WASM: 2496 non-color continuous label states, including 12 replacements.');
