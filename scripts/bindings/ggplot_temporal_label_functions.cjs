...
const cases=JSON.parse(fs.readFileSync(path.join(root,'fixtures/parity/ggplot2/temporal-label-functions.json'))).cases;
...
function descriptor(t,variant,multiplier){
 ...
 return {training:'Eligible',function:{Interpolated:{normalization:{Ggplot:{family:'Linear',domain:[0,1],reverse:false,rescaler:'Range',timestamp:{origin:String(origin),unit:variant,date}}},output:{Interpolate:{operation:'GgplotPalette',spec:{Gradient:{colors:[{red:19,green:43,blue:67,alpha:255},{red:86,green:177,blue:247,alpha:255}],values:null}}}},unknown:{kind:'Missing'}}},ggplot:{Continuous:{limits:null,oob:'Censor'}},guide:{Temporal:{origin:String(origin),unit:variant,zone:zone(t.zone),arguments:args}}};
}
...
function build(t,d,variant,multiplier){return c.plot(d).withRegistry(registry).profile('Ggplot2_4_0_3').aes(c.aes().x('x').y(1).color({field:'v',origin:String(BigInt(t.epoch)*multiplier)}).colorScale('v')).scale(c.colorMapped('v',descriptor(t,variant,multiplier))).layer(layer()).build();}
function check(t,chart){const state=chart.semantics().layers[0],entries=state.color_legend?.entries??[],expected=(t.result.keys??[]).flatMap(g=>g.labels.map(v=>v??'NA'));assert.deepEqual(entries.map(e=>e[0]),expected,JSON.stringify(t));return {entries,styles:state.styles??[]};}
...
for(const [index,t] of cases.entries())for(const [unit,variant,multiplier] of units){
 ...
 try{
  ...
  for(const state of ['original','edited']){
   ...
   const chart=current.chart();owned.push(chart);const actual=check(t,chart);assert.ok(!('error' in t.result),JSON.stringify(t));records.push({index,unit,state,...actual});
   if(state==='original'&&unit==='ms'&&sample(t)){const request=output.request(current,options);owned.push(request);const frame=request.prepare();owned.push(frame);for(const fmt of ['svg','pdf','png'])fs.writeFileSync(path.join(out,`sample-${index}.${fmt}`),frame.export(fmt));}
  }
...
for(const [kind,zoneName,epoch] of [['date','UTC',1704067200],['datetime','UTC',1710046800],['datetime','UTC',1730606400],['datetime','America/New_York',1710046800],['datetime','America/New_York',1730606400]]){
 ...
 try{
  ...
  for(const population of ['constant','missing','empty','spaced']){
   ...
   const fresh=build(t,replacement,'Milliseconds',1000n);owned.push(fresh);const batch=fresh.chart();owned.push(batch);const actual=check(t,chart);assert.deepEqual(actual,check(t,batch));assert.deepEqual(held.scene(),scene);assert.equal(p.toJson(),wire);records.push({kind,zone:zoneName,epoch,replacement:population,...actual});
  }
 }finally{for(const obj of owned.reverse())obj.dispose();}
}
assert.equal(records.length,2800);
fs.writeFileSync(path.join(out,'records.json'),JSON.stringify(records,null,2));options.dispose();output.dispose();registry.dispose();console.log('PASS WASM: 2800 temporal label states and 12 publication files.');
