...
for(const [index,testcase] of JSON.parse(fs.readFileSync(path.join(root,'fixtures/parity/ggplot2/temporal-aesthetic-precision.json'))).cases.entries()){
 ...
 try{
  ...
  const wire=JSON.parse(plot.to_json());assert.equal(wire.version,25);if(testcase.hidden)hide(wire);
  ...
  styles.forEach((style,i)=>{const value=testcase.values[i];if(channel==='alpha')assert.equal(style.color.alpha,Math.floor(value*255+0.5));else if(['size','linewidth'].includes(channel))assert.ok(Math.abs(style[channel==='size'?'radius':'stroke_width']-value)<2e-12,`${index} ${JSON.stringify(style)} ${value}`);else{const paint=style[channel==='fill'?'fill':'color'];assert.deepEqual([paint.red,paint.green,paint.blue],[1,3,5].map(i=>parseInt(value.slice(i,i+2),16)));}});
