...
input_trace = []
for case in json.loads((ROOT / 'fixtures/interaction/cases.json').read_text()):
    ...
    for step in case['queries']:
        ...
        if action is not None:
            ...
            chart.dispatch(json.dumps(dict(definition_revision=current['definition_revision'],expected_state=current['state_revision'],scene=basis or stamp,origin='Pointer',action=action)))
...
print(f'PASS WP-16 Python shared input queries/actions: {len(input_trace)} steps')
