...
class Component(_Owned):
    ...
    def __getattr__(self, name):
        ...
        def apply(*args, **kwargs):
            ...
            elif name == "recipe_value" and len(args) == 2:
                target, source = args
                if isinstance(source, Field): inner = self._inner.recipe_value_field(_encode(target), source._inner)
                elif isinstance(source, Component): inner = self._inner.recipe_value_expression(_encode(target), source._inner)
                else: inner = self._inner.set(name, _encode(args))
