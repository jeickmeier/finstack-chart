...
class Component extends Owned {
  constructor(inner){super(inner);return fluent(this,(t,name,args)=>{
    ...
    else if(name==='recipe_value'&&args.length===2){const [target,source]=args;if(source instanceof Field)next=t._inner.recipe_value_field(encode(target),source._inner);else if(source instanceof Component)next=t._inner.recipe_value_expression(encode(target),source._inner);else next=t._inner.set(name,shapeEncode(args));}
