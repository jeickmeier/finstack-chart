...
impl Component {
    ...
    pub fn new(name: &str, arguments: &str) -> ChartResult<Self> {
        ...
        Ok(Self(match name {
            ...
            "model_stat" => Kind::Stat(model_stat(a.one()?)),
            "spatial_stat" => Kind::Stat(spatial_stat(a.one()?)),
            "bin2d_stat" => Kind::Stat(bin2d_stat()),
            "hex_stat" => Kind::Stat(hex_stat()),
            "density2d_stat" => Kind::Stat(density2d_stat()),
            "contour_stat" => Kind::Stat(contour_stat()),
            "contour_filled_stat" => Kind::Stat(contour_filled_stat()),
            "ellipse_stat" => Kind::Stat(ellipse_stat()),
            "bin2d" => Kind::Layer(bin2d()),
            "hex" => Kind::Layer(hex()),
            "density2d" => Kind::Layer(density2d()),
            "contour" => Kind::Layer(contour()),
            "contour_filled" => Kind::Layer(contour_filled()),
            "ellipse" => Kind::Layer(ellipse()),
            "smooth_stat" => empty!(Stat, smooth_stat),
            "quantile_stat" => empty!(Stat, quantile_stat),
            "smooth" => empty!(Layer, smooth),
            "quantile" => empty!(Layer, quantile),
            ...
            "math_text" => {
                let (source, fonts) = a.pair::<String, crate::typography::MathFonts>()?;
                Kind::Rich(math_text(source, fonts)?)
            }
