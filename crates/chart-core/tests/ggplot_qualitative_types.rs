//! FIX-GG04: public qualitative type vectors select by category count.
use chart_core::{
...
#[test]
fn qualitative_type_vectors_match_80_reference_draws() {
    let fixture: Json = serde_json::from_str(include_str!(
        "../../../fixtures/parity/ggplot2/qualitative-type-palettes.json"
    ))
    .unwrap();
    ...
    for t in cases {
        let result = (|| -> ChartResult<_> {
            ...
            *palette = GgplotDiscretePalette::Qualitative {
                palettes: t["palette"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .map(|p| GgplotQualitativeColors {
                        values: serde_json::from_value(p["values"].clone()).unwrap(),
                        names: p["names"].as_array().map(|names| {
                            names
                                .iter()
                                .map(|v| ScaleKey::Text(v.as_str().unwrap().into()))
                                .collect()
                        }),
                    })
                    .collect(),
            ...
            assert_eq!(downgraded["version"], 50);
            downgraded["version"] = json!(49);
            assert_eq!(
