...
fn spec(c: &Json) -> MappedScaleSpec {
    let policy = GgplotBinnedPolicy {
        ...
        breaks: match c["guide_mode"].as_str() {
            Some("first") => GgplotBreaks::Explicit(vec![Number(1.)]),
            Some("restricted") => GgplotBreaks::Explicit([10., 1., 4.].map(Number).to_vec()),
    ...
    let mut result =
        ...
        .with_guide(if c["guide_mode"] == "hidden" {
            ...
            GgplotScaleGuide::Binned(GgplotGuideLabels::Automatic)
        })
        .unwrap()
...
#[test]
fn binned_vector_palettes_match_540_reference_builds() {
    let mut count = 0;
    for source in [
            ...
            .filter(|c| c["family"] == "binned")
        {
            ...
            let result = (|| -> ChartResult<Json> {
                ...
                let entries = scale.binned_guide_entries(4096, 65536)?.unwrap_or_default();
                let mut values = vec![];
                let mut labels = vec![];
                let mut colors = vec![];
                for e in entries.into_iter().filter(|e| e.visible) {
                    ...
                    labels.push(e.label);
                    colors.push(encode(scale.numeric(Some(e.value.0))?));
                }
                let mapped = inputs
                    .iter()
                    .map(|v| scale.numeric(v.map(|v| v.0)).map(encode))
...
#[test]
fn primary_binned_palette_roundtrip_matches_540_reference_outcomes() {
...
#[test]
fn binned_palette_missing_replacements_match_54_source_draws() {
    use chart_core::prelude::*;
...
#[test]
fn binned_palette_batches_match_81_vector_sensitive_reference_builds() {
    let fixture: Json = serde_json::from_str(include_str!(
        ...
        .filter(|c| c["family"] == "binned")
    {
        ...
        let layers = c["inputs"]
            ...
            .collect::<Vec<_>>();
        let population = layers.iter().flatten().copied().collect::<Vec<_>>();
        ...
        .unwrap();
        let mapped: Vec<_> = layers
            .iter()
            .map(|layer| {
                layer
                    .iter()
                    .map(|v| encode(scale.numeric(v.map(|v| v.0)).unwrap()))
                    .collect::<Vec<_>>()
            })
            .collect();
        compare(&json!(mapped), &c["result"]["mapped"]);
        let entries = scale
            .binned_guide_entries(4096, 65536)
            .unwrap()
            .unwrap_or_default();
        let mut keys = vec![];
        let mut labels = vec![];
        ...
        for e in entries.into_iter().filter(|e| e.visible) {
            keys.push(e.value.0);
            labels.push(e.label);
            values.push(encode(scale.numeric(Some(e.value.0)).unwrap()));
        }
        compare(
            &if keys.is_empty() {
                json!([])
