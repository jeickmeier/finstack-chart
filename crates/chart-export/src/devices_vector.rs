...
use crate::{PublicationProfile, error};
use chart_core::{
...
pub(super) fn append(
    ...
    Sink { text: data, limit }.write_fmt(args).map_err(|_| {
        error(
            DiagnosticCode::ResourceLimit,
            "PostScript exceeds the output byte budget.",
        )
    })
}
struct Writer {
...
impl Writer {
    fn add(&mut self, args: std::fmt::Arguments<'_>) -> ChartResult<()> {
        append(&mut self.data, self.limit, args)
    }
    fn alpha(&mut self, value: f32) -> ChartResult<bool> {
        if value == 0. {
            return Ok(false);
        }
        if value == 1. {
        ...
        match self.options.alpha {
            ...
            VectorAlphaPolicy::Reject => Err(error(
                ...
                "PostScript does not support partial transparency; select an explicit device alpha policy.",
            )),
        }
    }
        ...
        for command in p.segments() {
            match command {
                ...
                QuadTo(a, b) => {
                    self.add(format_args!(
                        "{} {} {} {} {} {} curveto\n",
                        current.0 + 2. / 3. * (a.x - current.0),
                        current.1 + 2. / 3. * (a.y - current.1),
                        b.x + 2. / 3. * (a.x - b.x),
                        b.y + 2. / 3. * (a.y - b.y),
                        b.x,
                        b.y
    ...
    fn paint(&mut self, paint: &usvg::Paint, stroke: bool, even: bool) -> ChartResult<()> {
        match paint {
            usvg::Paint::Color(c) => {
                self.color(*c)?;
                self.add(format_args!(
    ...
    fn node(&mut self, node: &usvg::Node) -> ChartResult<()> {
        match node {
            ...
            usvg::Node::Text(t) => self.group(t.flattened()),
            usvg::Node::Image(_) => Err(error(
                DiagnosticCode::ExportFidelity,
                "Image must use its typed publication raster payload.",
            )),
            usvg::Node::Path(p) => {
                if !p.is_visible() {
                    return Ok(());
                ...
                for stroke in order {
                    ...
                    self.transform(p.abs_transform())?;
                    self.path(p.data())?;
                    if stroke {
                        self.stroke(p.stroke().unwrap())?;
                    }
                    self.paint(paint, stroke, even)?;
    ...
    fn group(&mut self, g: &usvg::Group) -> ChartResult<()> {
        if !self.alpha(g.opacity().get())? {
            return Ok(());
        }
        ...
        if g.mask().is_some() || !g.filters().is_empty() {
            return Err(error(
                DiagnosticCode::ExportFidelity,
                "PostScript cannot silently flatten masks or filters.",
            ));
        }
...
pub(crate) fn postscript(
    scene: &Scene,
    tree: &usvg::Tree,
    p: &PublicationProfile,
    eps: bool,
) -> ChartResult<(Vec<u8>, usize)> {
    postscript_pages(&[(scene, tree, p)], eps)
}
pub(crate) fn postscript_pages(
    ...
    let mut w = Writer {
        data: String::new(),
        limit: pages.iter().map(|v| v.2.max_output_bytes).min().unwrap(),
    ...
    for (i, (scene, tree, p)) in pages.iter().enumerate() {
        ...
        page(&mut w, scene, tree, p, eps, i + 1)?;
    }
    let omitted = w.omitted;
    w.add(format_args!(
        "%%Trailer\n%%ChartOmittedTranslucentPaints: {}\n%%EOF\n",
        omitted
    ))?;
    Ok((w.data.into_bytes(), w.omitted))
}
fn page(
    w: &mut Writer,
    scene: &Scene,
            ...
            Primitive::RasterImage {
                ...
                if raster.pixels.iter().all(|c| c.alpha == 255) {
                    ...
                    for (i, c) in raster.pixels.iter().enumerate() {
                        if !w.alpha(f32::from(c.alpha) / 255.)? {
                            continue;
                        }
...
mod tests {
    ...
    fn pinned_reference_color_models_keep_calibration_and_neutral_policy() {
        ...
        for model in [VectorColorModel::SrgbGray, VectorColorModel::RgbGray] {
            ...
            assert!(w.data.ends_with(" setgray\n"));
        }
        let mut w = writer(VectorColorModel::Srgb);
        w.color(c).unwrap();
        assert!(w.data.starts_with("ChartSRGB setcolorspace\n"));
    }
    #[test]
    fn quadratic_paths_and_strokes_remain_vector_commands() {
        ...
        w.group(tree.root()).unwrap();
        assert!(w.data.contains("2 4 4 4 6 0 curveto"));
        assert!(w.data.contains("2 setlinewidth"));
        assert!(w.data.contains("1 setlinecap"));
        assert!(w.data.contains("2 setlinejoin"));
        assert!(w.data.contains("[3 2 ] 0 setdash"));
        assert!(!w.data.contains("image"));
    }
    #[test]
    fn gradient_shading_preserves_resolved_stop_intervals_and_endpoint_padding() {
        ...
        let mut w = writer(VectorColorModel::Rgb);
        w.group(tree.root()).unwrap();
        assert!(w.data.contains("/ShadingType 2"));
        assert!(
            w.data.contains("/Bounds [0.2 0.49999988 0.5 0.8 ]"),
            "{}",
            w.data
        );
        assert!(w.data.contains("clip newpath"));
        assert_eq!(w.data.matches("/FunctionType 2").count(), 5);
    }
    #[test]
    fn unsupported_alpha_is_explicit_and_writer_budget_prevents_growth() {
        let mut w = writer(VectorColorModel::Rgb);
        assert_eq!(
            w.alpha(0.5).unwrap_err().code,
            DiagnosticCode::ExportFidelity
        );
        assert!(!w.alpha(0.).unwrap());
        w.options.alpha = VectorAlphaPolicy::OmitTranslucent;
        assert!(!w.alpha(0.5).unwrap());
        w.limit = 2;
        assert_eq!(
