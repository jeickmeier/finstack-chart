//! FIX-GG10 independently authored Rust model publications.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..10 {
        ...
        std::fs::write(out.join(format!("model-{mode}.plot.json")), p.to_json()?)?;
        std::fs::write(
            out.join(format!("model-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("model-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust model controls: ten authors, 30 publications.");
    Ok(())
}
