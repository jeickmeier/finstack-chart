//! FIX-GG13 independently authored Rust coordinate publications.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..14 {
        ...
        std::fs::write(
            out.join(format!("coordinate-{mode}.plot.json")),
            p.to_json()?,
        )?;
        std::fs::write(
            out.join(format!("coordinate-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("coordinate-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust coordinate controls: 14 authors, 42 publications.");
    Ok(())
}
