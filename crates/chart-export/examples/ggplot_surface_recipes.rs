...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..9 {
        ...
        std::fs::write(out.join(format!("surface-{mode}.plot.json")), p.to_json()?)?;
        std::fs::write(
            out.join(format!("surface-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("surface-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust surfaces: nine authors, 27 publications.");
    Ok(())
}
