//! FIX-GG14 independently authored vector and extension publications.
#[path = "../../../examples/common/ggplot_furniture_controls.rs"]
mod fixtures;
use chart_export::*;
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..fixtures::CASES {
        ...
        std::fs::write(
            out.join(format!("furniture-{mode}.plot.json")),
            p.to_json()?,
        )?;
        std::fs::write(
            out.join(format!("furniture-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("furniture-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust furniture controls: eight authors, 24 publications.");
    Ok(())
}
