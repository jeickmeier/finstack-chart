//! FIX-GG18 independently authored vector and extension publications.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..fixtures::CASES {
        ...
        std::fs::write(
            out.join(format!("integration-{mode}.plot.json")),
            p.to_json()?,
        )?;
        std::fs::write(
            out.join(format!("integration-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("integration-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust integration controls: four authors, twelve publications.");
    Ok(())
}
