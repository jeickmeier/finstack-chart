...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in 0..15 {
        ...
        std::fs::write(
            out.join(format!("distribution-{mode}.plot.json")),
            p.to_json()?,
        )?;
        std::fs::write(
            out.join(format!("distribution-{mode}.scene.json")),
            frame.scene_json()?,
        )?;
        for (format, suffix) in [
            ...
            std::fs::write(
                out.join(format!("distribution-{mode}.{suffix}")),
                frame.export(format)?.bytes,
            )?;
        }
    }
    println!("PASS Rust distribution geometry: fifteen authors, 45 publications.");
    Ok(())
}
