...
//        (self.abs() + ((self * self) + 1.0).portable_sqrt()).portable_ln().copysign(self)
//  }
// ```
//
...
//     (y * (1. + y/(hypot(1.0,y) + 1.))).portable_ln_1p().copysign(x)
// }
// ```
...
//pub fn asinh(x: f64) -> f64 {
//    let y = x.abs();         // Enforce odd parity
//    (y * (1. + y/(1.0f64.portable_hypot(y) + 1.))).portable_ln_1p().copysign(x)
//}
...
pub(crate) use gauss::{Gauss, gauss, gauss_ini, inv_gauss};
pub(crate) use mlfn::{Enfn, enfn, inv_mlfn, mlfn};
