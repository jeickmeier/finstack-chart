...
impl Projection {
    pub fn cea(p: &mut ProjData, params: &ParamList) -> Result<Self> {
        ...
        Ok(if p.ellps.is_ellipsoid() {
            let sint = t.portable_sin();
            k0 /= (1. - p.ellps.es * sint * sint).portable_sqrt();
            Self::Ell {
                k0,
                e: p.ellps.e,
    ...
    pub fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        match self {
            Self::Ell { k0, e, one_es, .. } => Ok((
                k0 * lam,
                0.5 * qsfn(phi.portable_sin(), *e, *one_es) / k0,
                z,
            )),
            Self::Sph { k0 } => Ok((k0 * lam, phi.portable_sin() / k0, z)),
        }
    }
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        match self {
            Self::Ell { k0, qp, apa, .. } => {
                Ok((x / k0, authlat((2. * y * k0 / qp).portable_asin(), *apa), z))
            }
            Self::Sph { k0 } => {
                let y = y * k0;
                let t = y.abs();
