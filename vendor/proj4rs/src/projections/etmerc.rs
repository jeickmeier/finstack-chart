...
fn gatg(c: &Coeffs, B: f64) -> f64 {
    ...
    let h = c
        ...
        .unwrap();
    B + h * (2. * B).portable_sin()
}

// Complex Clenshaw summation
#[inline]
fn clens_cplx(a: &Coeffs, arg_r: f64, arg_i: f64) -> (f64, f64) {
    let (sin_arg_r, cos_arg_r) = arg_r.portable_sin_cos();
    let sinh_arg_i = arg_i.portable_sinh();
    let cosh_arg_i = arg_i.portable_cosh();
...
fn clens(a: &Coeffs, arg_r: f64) -> f64 {
    let cos_arg_r = arg_r.portable_cos();
    let r = 2. * cos_arg_r;
...
impl Projection {
    ...
    pub fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let (sin_Cn, cos_Cn) = Cn.portable_sin_cos();
        let (sin_Ce, cos_Ce) = Ce.portable_sin_cos();
        
        Cn = sin_Cn.portable_atan2(cos_Ce*cos_Cn);
        Ce = (sin_Ce*cos_Cn).portable_atan2(sin_Cn.portable_hypot(cos_Cn*cos_Ce));

        // compl. sph. N, E -> ell. norm. N, E
        Ce  = asinh(Ce.portable_tan());
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        if Ce.abs() <= 2.623395162778 { // 150 degrees
            ...
            Cn += dCn;
            Ce += dCe;
            Ce = Ce.portable_sinh().portable_atan(); 
            // compl. sph. LAT -> Gaussian LAT, LNG
            let (sin_Cn, cos_Cn) = Cn.portable_sin_cos();
            let (sin_Ce, cos_Ce) = Ce.portable_sin_cos();

            Ce = sin_Ce.portable_atan2(cos_Ce*cos_Cn);
            Cn = (sin_Cn*cos_Ce).portable_atan2(sin_Ce.portable_hypot(cos_Ce*cos_Cn));
