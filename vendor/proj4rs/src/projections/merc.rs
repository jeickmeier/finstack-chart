...
impl Projection {
    pub fn merc(p: &mut ProjData, params: &ParamList) -> Result<Self> {
        ...
        if p.ellps.is_ellipsoid() {
            if let Some(phits) = phits {
                p.k0 = msfn(phits.portable_sin(), phits.portable_cos(), p.ellps.es);
            }
        } else if let Some(phits) = phits {
            p.k0 = phits.portable_cos();
        }
    ...
    pub fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        if self.is_ellps {
            let (sphi, cphi) = phi.portable_sin_cos();
            Ok((
                self.k0 * lam,
                self.k0 * (asinh(sphi / cphi) - self.e * (self.e * sphi).portable_atanh()),
                z,
            ))
        } else {
            Ok((self.k0 * lam, self.k0 * asinh(phi.portable_tan()), z))
        }
    }

    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        if self.is_ellps {
            Ok((x / self.k0, phi2((-y / self.k0).portable_exp(), self.e)?, z))
        } else {
            Ok((
                x / self.k0,
                (y / self.k0).portable_sinh().portable_atan(),
                z,
            ))
        }
    }
