...
impl Projection {
    ...
    fn new(p: &mut ProjData, pp: f64) -> Result<Self> {
        ...
        let p2 = pp + pp;
        let sp = pp.portable_sin();
        let c_p = p2 + p2.portable_sin();
        let r = (TAU * sp / c_p).portable_sqrt();
    ...
    pub fn forward(&self, lam: f64, mut phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let k = self.c_p * phi.portable_sin();
        let mut i = NITER;
        while i > 0 {
            let v = (phi + phi.portable_sin() - k) / (1. + phi.portable_cos());
            phi -= v;
            if v.abs() < TOL {
                break;
            }
        ...
        Ok((
            self.c_x * lam * phi.portable_cos(),
            self.c_y * phi.portable_sin(),
            z,
        ))
    }
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let lam = x / (self.c_x * phi.portable_cos());
        if lam.abs() < PI {
            phi += phi;
            phi = aasin((phi + phi.portable_sin()) / self.c_p)?;
            Ok((lam, phi, z))
        } else {
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        let mut phi = aasin(y / self.c_y)?;
        let mut lam = x / (self.c_x * phi.portable_cos());
        if lam.abs() > PI {
            lam = PI * lam.signum();
        }
        phi += phi;
