    ...
    pub fn estmerc(p: &mut ProjData, _: &ParamList) -> Result<Self> {
        if p.ellps.is_ellipsoid() {
            ...
            Ok(Ell(Ell {
                ...
                es,
                esp: es / (1. - es),
                ml0: mlfn(p.phi0, p.phi0.portable_sin(), p.phi0.portable_cos(), en),
            }))
        } else {
            Ok(Sph(Sph {
                phi0: p.phi0,
...
impl Ell {
    fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let als = al * al;
        al /= (1. - self.es * sinphi * sinphi).portable_sqrt();
    ...
    fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        if phi.abs() >= FRAC_PI_2 {
            ...
            let d = x * con.portable_sqrt() / self.k0;
            con *= t;
            t *= t;
            let ds = d * d;
            Ok((
...
impl Sph {
    fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let mut b = cosphi * lam.portable_sin();
        if (b.abs() - 1.).abs() <= EPS_10 {
            return Err(Error::ToleranceConditionError);
        }

        let x = self.ml0 * ((1. + b) / (1. - b)).portable_ln();
        let mut y = cosphi * lam.portable_cos() / (1. - b * b).portable_sqrt();
    ...
    fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        let mut h = (x / self.esp).portable_exp();
        let g = 0.5 * (h - 1. / h);

        h = (self.phi0 + y / self.esp).portable_cos();
        let mut phi = ((1. - h * h) / (1. + g * g))
            .portable_sqrt()
            .portable_asin();
