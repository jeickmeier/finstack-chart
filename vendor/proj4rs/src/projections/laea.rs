...
impl EProj {
    fn new(p: &mut ProjData) -> Self {
        ...
        let mode = if (t - FRAC_PI_2).abs() < EPS_10 {
            ...
            let (sinphi, cosphi) = phi0.portable_sin_cos();
            let rq = (0.5 * qp).portable_sqrt();
            let sinb1 = qsfn(sinphi, e, one_es) / qp;
            let cosb1 = (1. - sinb1 * sinb1).portable_sqrt();
            let dd = cosphi / ((1. - p.ellps.es * sinphi * sinphi).portable_sqrt() * rq * cosb1);
            OBLIQ {
                rq,
                dd,
                sinb1,
    ...
    fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let q = qsfn(phi.portable_sin(), self.e, self.one_es);
        ...
        let (x, y) = match self.mode {
            OBLIQ {
                ...
                let sinb = q / self.qp;
                let cosb = (1. - sinb * sinb).portable_sqrt();
    ...
    fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let (ab, xx, yy) = match self.mode {
            EQUIT { dd, rq, .. } => {
                ...
                let (sce, cce) = (2. * (0.5 * rho / rq).portable_asin()).portable_sin_cos();
        ...
        Ok((
            ...
            authlat(ab.portable_asin(), self.apa),
            z,
        ))
    }
}
...
impl SProj {
    fn new(p: &mut ProjData) -> Self {
        ...
        let mode = if (t - FRAC_PI_2).abs() < EPS_10 {
            ...
            let (sinb1, cosb1) = p.phi0.portable_sin_cos();
            OBLIQ { sinb1, cosb1 }
        };
        Self { phi0, mode }
    }
        ...
        match self.mode {
            EQUIT => {
                ...
                if y < EPS_10 {
                    ...
                    y = (2. / y).portable_sqrt();
                    Ok((y * cosphi * lam.portable_sin(), y * sinphi, z))
                }
            }
            OBLIQ { sinb1, cosb1, .. } => {
            ...
            N_POLE | S_POLE => {
                if (phi + self.phi0).abs() < EPS_10 {
                    ...
                    if matches!(self.mode, N_POLE) {
                        y = 2. * y.portable_sin();
                        Ok((y * lam.portable_sin(), y * -coslam, z))
                    } else {
                        y = 2. * y.portable_cos();
                        Ok((y * lam.portable_sin(), y * coslam, z))
                    }
    ...
    fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        phi = 2. * phi.portable_asin();
        match self.mode {
            EQUIT => {
                let (sinz, cosz) = phi.portable_sin_cos();
                phi = if rh <= EPS_10 {
                    0.
                } else {
                    (y * sinz / rh).portable_asin()
                };
                let yy = cosz * rh;
                lam = if yy == 0. {
                    0.
                } else {
                    (x * sinz).portable_atan2(yy)
                };
            }
            OBLIQ { sinb1, cosb1 } => {
                let (sinz, cosz) = phi.portable_sin_cos();
                phi = if rh <= EPS_10 {
                    ...
                    (cosz * sinb1 + y * sinz * cosb1 / rh).portable_asin()
                };
                let yy = (cosz - phi.portable_sin() * sinb1) * rh;
                lam = if yy == 0. {
                    0.
                } else {
                    (x * sinz * cosb1).portable_atan2(yy)
                };
            }
            N_POLE => {
                phi = FRAC_PI_2 - phi;
                lam = x.portable_atan2(-y);
            }
            S_POLE => {
                phi -= FRAC_PI_2;
                lam = x.portable_atan2(y);
