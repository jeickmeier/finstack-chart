...
fn ssfn(phit: f64, sinphi: f64, eccen: f64) -> f64 {
    ...
    (0.5 * (FRAC_PI_2 + phit)).portable_tan()
        * ((1. - sinphi) / (1. + sinphi)).portable_powf(0.5 * eccen)
}
...
impl Projection {
    ...
    pub fn e_forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        let coslam = lam.portable_cos();
        let sinlam = lam.portable_sin();
        let sinphi = phi.portable_sin();

        let (x, y) = match self.mode {
            OBLIQ => {
                let x = 2. * ssfn(phi, sinphi, self.e).portable_atan() - FRAC_PI_2;
                let (sinx, cosx) = x.portable_sin_cos();
    ...
    pub fn e_inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let (xx, yy) = match self.mode {
            OBLIQ | EQUIT => {
                let (sinphi, cosphi) =
                    (2. * (rho * self.cosx1).portable_atan2(self.akm1)).portable_sin_cos();
                phi_l = if rho == 0.0 {
                    (cosphi * self.sinx1).portable_asin()
                } else {
                    (cosphi * self.sinx1 + (y * sinphi * self.cosx1 / rho)).portable_asin()
                };
                tp = (0.5 * (FRAC_PI_2 + phi_l)).portable_tan();
                halfpi = FRAC_PI_2;
                halfe = 0.5 * self.e;
                (
                    x * sinphi,
            ...
            _ => {
                tp = -rho / self.akm1;
                phi_l = FRAC_PI_2 - 2. * tp.portable_atan();
                halfpi = -FRAC_PI_2;
                halfe = -0.5 * self.e;
        ...
        let mut i = NITER;
        while i > 0 {
            let sinphi = self.e * phi_l.portable_sin();
            phi = 2. * (tp * ((1. + sinphi) / (1. - sinphi)).portable_powf(halfe)).portable_atan()
                - halfpi;
    ...
    pub fn s_forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        let (sinphi, cosphi) = phi.portable_sin_cos();
        let (sinlam, coslam) = lam.portable_sin_cos();
        ...
        let (x, y) = match mode {
            ...
            _ => {
                ...
                let y = self.akm1 * (FRAC_PI_4 + 0.5 * phi).portable_tan();
                (y * sinlam, y * coslam)
            }
        };
        Ok((x, y, z))
    ...
    pub fn s_inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        let rh = x.portable_hypot(y);

        let (sinc, cosc) = (2. * (rh / self.akm1).portable_atan()).portable_sin_cos();
        let (lam, phi) = match self.mode {
            EQUIT => (
                // lam
                if cosc != 0. || x != 0. {
                    (x * sinc).portable_atan2(cosc * rh)
                } else {
                    0.
                },
                // phi
            ...
            OBLIQ => {
                let phi = if rh.abs() <= EPS_10 {
                    ...
                    (cosc * self.sinx1 + y * sinc * self.cosx1 / rh).portable_asin()
                };
                let c = cosc - self.sinx1 * phi.portable_sin();
                (
                    // lam
                    if c != 0. || x != 0. {
                        (x * sinc * self.cosx1).portable_atan2(c * rh)
                    } else {
                        0.
                    },
                    phi,
            ...
            match mode {
                N_POLE | S_POLE => {
                    if (phits - FRAC_PI_2).abs() < EPS_10 {
                        2. * p.k0
                            / ((1. + ecc).portable_powf(1. + ecc)
                                * (1. - ecc).portable_powf(1. - ecc))
                            .portable_sqrt()
                    } else {
                        let s = phits.portable_sin();
                        let t = s * ecc;
                        phits.portable_cos() / tsfn(phits, s, ecc) / (1. - t * t).portable_sqrt()
                    }
                }
                EQUIT | OBLIQ => {
                    let mut t = p.phi0.portable_sin();
                    let x = 2. * ssfn(p.phi0, t, ecc).portable_atan() - FRAC_PI_2;
                    sinx1 = x.portable_sin();
                    cosx1 = x.portable_cos();
                    t *= ecc;
                    2. * p.k0 * p.phi0.portable_cos() / (1. - t * t).portable_sqrt()
                }
            }
        } else {
            match mode {
                EQUIT => 2. * p.k0,
                OBLIQ => {
                    sinx1 = p.phi0.portable_sin();
                    cosx1 = p.phi0.portable_cos();
                    2. * p.k0
                }
                S_POLE | N_POLE => {
                    if (phits - FRAC_PI_2).abs() >= EPS_10 {
                        phits.portable_cos() / (FRAC_PI_4 - 0.5 * phits).portable_tan()
                    } else {
                        2. * p.k0
                    }
                }
