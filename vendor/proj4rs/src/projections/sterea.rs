...
impl Projection {
    pub fn sterea(p: &mut ProjData, _: &ParamList) -> Result<Self> {
        ...
        let (sinc0, cosc0) = phic0.portable_sin_cos();
        Ok(Self {
            k0: p.k0,
            phic0,
            cosc0,
    ...
    pub fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let (sinc, cosc) = phi.portable_sin_cos();
        let cosl = lam.portable_cos();
        let k = self.k0 * self.r2 / (1. + self.sinc0 * sinc + self.cosc0 * cosc * cosl);
        Ok((
            k * cosc * lam.portable_sin(),
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let (lam, phi) = if rho != 0.0 {
            let c = 2. * rho.portable_atan2(self.r2);
            let (sinc, cosc) = c.portable_sin_cos();
            inv_gauss(
                (x * sinc).portable_atan2(rho * self.cosc0 * cosc - y * self.sinc0 * sinc),
                (cosc * self.sinc0 + y * sinc * self.cosc0 / rho).portable_asin(),
                &self.en,
            )
        } else {
