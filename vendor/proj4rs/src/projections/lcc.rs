...
impl Projection {
    pub fn lcc(p: &mut ProjData, params: &ParamList) -> Result<Self> {
        ...
        let sinphi = phi1.portable_sin();
        let cosphi = phi1.portable_cos();
        ...
        if ellips {
            ...
            // secant zone
            n = if secant {
                let sinphi2 = phi2.portable_sin();
                (m1 / msfn(sinphi2, phi2.portable_cos(), el.es)).portable_ln()
                    / (ml1 / tsfn(phi2, sinphi2, el.e)).portable_ln()
            } else {
                sinphi
            };
            //rho0 = m1 * ml1.portable_powf(-n);
            //c = rho0 / n;
            c = m1 * ml1.portable_powf(-n) / n;
            rho0 = if (phi0.abs() - FRAC_PI_2).abs() < EPS_10 {
                0.
            } else {
                c * tsfn(phi0, phi0.portable_sin(), el.e).portable_powf(n)
            }
        } else {
            n = if secant {
                (cosphi / phi2.portable_cos()).portable_ln()
                    / ((FRAC_PI_4 + 0.5 * phi2).portable_tan()
                        / (FRAC_PI_4 + 0.5 * phi1).portable_tan())
                    .portable_ln()
            } else {
                sinphi
            };
            c = cosphi * (FRAC_PI_4 + 0.5 * phi1).portable_tan().portable_powf(n) / n;
            rho0 = if (phi0.abs() - FRAC_PI_2).abs() < EPS_10 {
                0.
            } else {
                c * (FRAC_PI_4 + 0.5 * phi0).portable_tan().portable_powf(-n)
            }
        }
    ...
    pub fn forward(&self, mut lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        let rho = if (phi.abs() - FRAC_PI_2).abs() < EPS_10 {
            ...
            self.c
                * if self.ellips {
                    tsfn(phi, phi.portable_sin(), self.e).portable_powf(self.n)
                } else {
                    (FRAC_PI_4 + 0.5 * phi)
                        .portable_tan()
                        .portable_powf(-self.n)
                }
        };
        ...
        Ok((
            self.k0 * (rho * lam.portable_sin()),
            self.k0 * (self.rho0 - rho * lam.portable_cos()),
            z,
        ))
    }
    ...
    pub fn inverse(&self, mut x: f64, mut y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        // one given in proj4
        let mut rho = x.portable_hypot(y);
        let (lam, phi);
        if rho != 0. {
            if self.n < 0. {
                rho = -rho;
            ...
            phi = if self.ellips {
                phi2((rho / self.c).portable_powf(1. / self.n), self.e)?
            } else {
                2. * (self.c / rho).portable_powf(1. / self.n).portable_atan() - FRAC_PI_2
            };
            lam = x.portable_atan2(y) / self.n;
        } else {
            lam = 0.;
