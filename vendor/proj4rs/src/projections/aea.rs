...
fn phi1_inv(qs: f64, e: f64, one_es: f64) -> Result<f64> {
    let mut phi = (0.5 * qs).portable_asin();
    if e < EPS_7 {
        Ok(phi)
    } else {
        let mut i = PHI_NITER;
        ...
        while i > 0 {
            (sinphi, cosphi) = phi.portable_sin_cos();
            con = e * sinphi;
            com = 1. - con * con;
            dphi = 0.5 * com * com / cosphi
                * (qs / one_es - sinphi / com + 0.5 / e * ((1. - con) / (1. + con)).portable_ln());
            phi += dphi;
...
impl Projection {
    pub fn init(p: &ProjData, phi1: f64, phi2: f64) -> Result<Self> {
        ...
        let el = &p.ellps;
        let (sinphi, cosphi) = phi1.portable_sin_cos();
        let mut n = sinphi;
        ...
        if el.is_ellipsoid() {
            ...
            if secant {
                let (sinphi2, cosphi2) = phi2.portable_sin_cos();
            ...
            let ec = 1. - 0.5 * el.one_es * ((1. - el.e) / (1. + el.e)).portable_ln() / el.e;
            let c = m1 * m1 + n * ml1;
            let dd = 1. / n;
            let n2 = n + n;
            let rho0 = dd * (c - n * qsfn(p.phi0.portable_sin(), el.e, el.one_es)).portable_sqrt();
            ...
            if secant {
                n = 0.5 * (n + phi2.portable_sin());
            }
            let dd = 1. / n;
            let n2 = n + n;
            let c = cosphi * cosphi + n2 * sinphi;
            let rho0 = dd * (c - n2 * p.phi0.portable_sin()).portable_sqrt();
            Ok(Self {
                e: el.e,
                one_es: el.one_es,
                ec: 1.,
    ...
    pub fn forward(&self, lam: f64, phi: f64, z: f64) -> Result<(f64, f64, f64)> {
        let rho = self.c
            - if self.is_ellipse() {
                self.n * qsfn(phi.portable_sin(), self.e, self.one_es)
            } else {
                self.n2 * phi.portable_sin()
            };

        if rho < 0. {
            Err(Error::ToleranceConditionError)
        } else {
            let rho = self.dd * rho.portable_sqrt();
            let (sin_i, cos_i) = (lam * self.n).portable_sin_cos();
    ...
    pub fn inverse(&self, x: f64, y: f64, z: f64) -> Result<(f64, f64, f64)> {
        ...
        let mut rho = xx.portable_hypot(yy);
        if rho != 0. {
            if self.n < 0. {
                rho = -rho;
                xx = -xx;
            ...
            Ok((xx.portable_atan2(yy) / self.n, phi, z))
        } else {
