...
pub fn geodetic_to_geocentric(x: f64, y: f64, z: f64, a: f64, es: f64) -> Result<(f64, f64, f64)> {
    ...
    let (sin_lat, cos_lat) = lat.portable_sin_cos();
    // Earth radius at location
    let rn = a / (1. - es * (sin_lat * sin_lat)).portable_sqrt();
    Ok((
        (rn + z) * cos_lat * lon.portable_cos(),
        (rn + z) * cos_lat * lon.portable_sin(),
        ((rn * (1. - es)) + z) * sin_lat,
    ))
}
...
pub fn geocentric_to_geodetic(
    ...
    let p = d2.portable_sqrt();
    // distance between center and location
    let rr = (d2 + z * z).portable_sqrt();
    ...
    let ct = z / rr;
    let st = p / rr;
    let mut rx = 1.0 / (1.0 - es * (2.0 - es) * st * st).portable_sqrt();
    ...
    loop {
        iter += 1;
        rn = a / (1.0 - es * sphi0 * sphi0).portable_sqrt();
        ...
        rk = es * rn / (rn + height);
        rx = 1.0 / (1.0 - rk * (2.0 - rk) * st * st).portable_sqrt();
        cphi = st * (1.0 - rk) * rx;
        sphi = ct * rx;
    ...
    Ok((lon, sphi.portable_atan2(cphi.abs()), height))
}
