...
| `gpui-charts` | GPUI element/entity integration, paint/text bridge, events, focus, host accessibility. | A second statistical/compiler engine or Kit types. |
| `chart-gallery` | Executable examples, visual fixtures, stress scenarios, optional Kit host recipes. | Production dependencies of the library crates. |
...
| AUT-06 — Destinations | Native and export adapters MUST accept the same authored plot/runtime contracts. Optional Kit hosts map theme tokens into those adapters; they are not a library crate. Retain native mounts and supplied resource contexts; offer concise bytes export and explicit host save/capture controls. No compulsory filesystem, system-font lookup, interpreter, GPUI object or threading enters core. Static and live capture semantics remain distinguishable and reproducible. |
