...
| Native | `NativeFont::load`, `ChartInput`, retained `ChartView`. Optional Kit theme snapshots are a gallery recipe, not a library crate. Hosts own window/input/control closures. [Standalone example](../examples/chart-gallery/src/main.rs), [composition examples](../examples/chart-gallery/examples/family_gallery.rs). |
