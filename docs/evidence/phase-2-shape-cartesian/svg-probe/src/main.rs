// Register the exact supplied font under its family name for resvg, which ignores SVG @font-face.
fn main() {
 ...
 let svg = std::fs::read_to_string(&a[1]).unwrap().replace("ChartFont-1-0", "Noto Sans");
