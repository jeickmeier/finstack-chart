...
WP-05 completion is `d0a6c48` plus uncommitted grammar/state modules, tests and documentation.

## Current handoff

The user assigned WP-05 after WP-04. **WP-05 is DONE for its grammar/prepared-scene scope.**
Typed native data and heterogeneous composed layers use one compiler with identity and
explicit-edge bin statistics, stage-safe generated mappings, point/line/rule/rectangle
geometry, endpoint domains and exact source/aggregate targets. Named DAG outputs reuse
immutable row arrays; operation metadata and calculation space are retained. Minimal
viewport/visibility/reset actions preserve the statistical population.

[The WP-05 report](evidence/wp-05-completion-2026-09-06.md) records FIX-01/02 known semantics
and prepared geometry, the composite histogram/annotation fixture, graph/cache validation,
precision and ownership checks. [ADR-002](adr/002-minimal-core-contracts.md) now records the
WP-05 authoring/compiler decisions and WP-06 handoff. Prepared geometry is in data/calculation
units; scale/range projection and text-aware layout remain WP-06, not implied paint output.

`mise run fmt`, `mise run check` and `mise run test` pass on macOS arm64: **59 core tests
(2 unit, 57 integration) and 2 Rustdoc examples**. Core remains dependency-free and synchronous.
No dependencies or capability assets changed. Core WASM compilation passes; actual wire
encoding and Python/WASM runtimes remain WP-09. Full grammar/statistical families, destination
chart rendering/export, complete interaction and G1–G4 remain open.
...
**Next:** WP-06 foundational scales, ticks and layout is READY. WP-07–09 still require
remaining prerequisites. This assignment stops at WP-05. Full stats/positions/facets,
selection events, time-window ingestion and sustained-load performance remain later work.
Licensing/public registry names remain release preparation decisions in ADR-010.
...
| WP-05 — Grammar compiler and minimal prepared scene | DONE | Unassigned | `d0a6c48` + uncommitted WP-05 slice | GRA-01, GRA-02, GRA-03, GRA-04, GRA-06, GRA-08, SCN-01, SCN-02, DAT-06 | [Completion evidence](evidence/wp-05-completion-2026-09-06.md); [ADR-002](adr/002-minimal-core-contracts.md) | Known data-space geometry/semantics accepted; scales/layout and destination scene projection are WP-06; complete grammar/extensions remain later packages. |
| WP-06 — Foundational scales, ticks and layout | READY | Unassigned | — | SCL-01, SCL-02, SCL-04, SCL-05, LAY-01, LAY-02, DAT-05 | None | WP-05 prerequisites satisfied; consume prepared endpoints/spaces for scale, tick and layout policy. |
...
| G1 | NOT PASSED | WP-04 data and WP-05 grammar evidence available; scales/layout, real native chart, headless chart output and Python/WASM runtime fixtures still required. |
