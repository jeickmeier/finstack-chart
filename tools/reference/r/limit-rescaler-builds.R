...
for(kind in c('continuous','binned'))for(rescaler in c('range','maximum','midpoint'))for(population in names(sets))for(control in c('identity','reverse','fixed','lower_zero','missing_lower','empty','single')){
 ...
 result<-tryCatch(suppressWarnings({
  ...
  built<-ggplot_build(ggplot(data.frame(x=seq_along(inputs),v=inputs),aes(x,1,size=v))+geom_point()+scale)
  list(values=encode(built$data[[1]]$size),keys=unname(lapply(built$plot$guides$params,function(g)list(values=encode(g$key$.value),labels=encode(g$key$.label),mapped=encode(g$key$size)))))
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/limit-rescaler-builds.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat('PASS',length(cases),'actual numeric limit rescaler builds\n')
