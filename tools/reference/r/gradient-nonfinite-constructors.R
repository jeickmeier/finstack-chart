...
for(channel in c('colour','fill'))for(constructor in c('gradientn','stepsn','distiller','viridis_c'))for(configuration in c('nan_middle','positive_infinity','negative_infinity','both_infinities','nan_leading','infinite_duplicate','single_negative_infinity'))for(population in c('ordinary','missing','empty')){
 if(configuration=='single_negative_infinity' && !constructor %in% c('gradientn','stepsn'))next
 inputs<-switch(population,ordinary=c(-4,-1,0,2,6),missing=c(-2,NA,2),empty=numeric())
 args<-list(values=switch(configuration,nan_middle=c(0,NA,1),positive_infinity=c(0,1,Inf),negative_infinity=c(-Inf,0,1),both_infinities=c(-Inf,0,1,Inf),nan_leading=c(NA,0,1),infinite_duplicate=c(0,Inf,Inf),single_negative_infinity=c(-Inf,.5,1)))
 if(constructor %in% c('gradientn','stepsn'))args$colours<-if(configuration=='single_negative_infinity')'red'else c('red','white','blue')
...
invisible(dev.off())
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/gradient-nonfinite-constructors.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat('PASS',length(cases),'nonfinite gradient remapping constructor draws\n')
