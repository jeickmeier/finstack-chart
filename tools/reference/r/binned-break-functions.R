# FIX-GG04: binned break functions, optional counts, names, and label inputs.
     ...
     for(count in if(default_names)'three'else c('none','three','zero'))
      for(mode in if(default_names)'mixed'else c('domain','mixed','empty','null')) {
       ...
       result <- tryCatch(suppressWarnings({
        ...
        scale <- if(channel=='size')do.call(binned_scale,c(list(aesthetics='size',palette=scales::pal_area(c(1,6))),args))else do.call(scale_colour_steps,args)
        ...
        result <- list(keys=unname(keys),mapped=encode(built$data[[1]][[channel]]))
        if(channel=='size') {
         parsed <- ggplot2:::parse_binned_breaks(built$plot$scales$get_scales(channel))
         result$boundaries <- if(is.null(parsed))list()else encode(sort(unique(c(parsed$limits,parsed$breaks)),na.last=NA))
        }
        result
       }),error=function(e)list(error=conditionMessage(e)))
       cases[[length(cases)+1L]] <- list(channel=channel,constructor=if(channel=='size')'binned_scale'else'scale_colour_steps',transform=tr,population=pop,limits=limits,signature=signature,count=count,mode=mode,inputs=encode(values),calls=calls,label_calls=label_calls,result=result)
      }
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),if(default_names)'fixtures/parity/ggplot2/binned-break-default-names.json'else'fixtures/parity/ggplot2/binned-break-functions.json',auto_unbox=TRUE,pretty=TRUE,digits=NA,null='null')
cat('PASS',length(cases),'binned break function reference builds\n')
