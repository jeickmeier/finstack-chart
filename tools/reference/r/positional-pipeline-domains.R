...
cases <- list()
for (limit_mode in c('automatic', 'full', 'constant'))
 for (axis in c('x', 'y'))
 for (transform in c('identity', 'reverse', 'log10'))
  for (population in c('ordinary', 'missing', 'empty'))
   for (mode in c('default', 'reverse', 'index', 'short')) {
    ...
    result <- tryCatch(suppressWarnings({
      scale <- do.call(if (axis == 'x') scale_x_continuous else scale_y_continuous,
                       list(limits = switch(limit_mode, automatic = NULL, full = c(1, 10), constant = c(4, 4)), transform = transform, oob = oob))
    ...
    cases[[length(cases) + 1L]] <- list(limit_mode = limit_mode, axis = axis, transform = transform,
