...
for (family in c('identity', 'reverse', 'log10'))
 for (kind in c('point_x', 'mean'))
  for (control in c('identity','reverse','fixed','single'))
   for (cuts in c('nice','domain','mixed'))
   for (population in c('ordinary','constant','missing','empty'))
    for (mode in c('default', 'reverse', 'index', 'short', 'empty', 'null')) {
     inputs <- switch(population, ordinary = c(-2, 1, 4, 10, 14),
       constant = c(4,4), missing = c(NA, 1, Inf, 4, -Inf), empty = numeric())
     groups <- rep(c(1, 1, 2, 2, 3), length.out = length(inputs))
     axis <- if (kind == 'point_x') 'x' else 'y'
     calls <- list(); limit_calls <- list(); break_calls <- list()
     limits <- function(x) {
      limit_calls[[length(limit_calls)+1L]] <<- encode(x)
      switch(control, identity=x, reverse=rev(x), fixed=c(1,10), single=5)
     }
     breaks <- function(x,n=7) {
      break_calls[[length(break_calls)+1L]] <<- list(values=encode(x),count=n)
      if (cuts=='domain') x else c(x[2],mean(x),x[1],x[1],NA,Inf,-Inf)
     }
     ...
     result <- tryCatch(suppressWarnings({
      ...
      args <- list(oob = oob, transform = family, n.breaks = 3, limits=limits)
      if (cuts != 'nice') args$breaks <- breaks
     ...
     cases[[length(cases) + 1L]] <- list(family = family, kind = kind, axis = axis,
       cuts = cuts, population = population, limit_mode = "automatic", control=control, mode = mode,
       inputs = encode(inputs), groups = encode(groups), calls = calls, limit_calls=limit_calls, break_calls=break_calls, result = result)
    }
stopifnot(length(cases) == 1728L)
