 ...
 for(channel in c('colour','size','alpha'))
  for(population in c('ordinary','missing'))
   for(mode in c('default','oob_reverse','oob_index','rescale_reverse','rescale_index','rescale_short','rescale_empty','rescale_null'))
    for(guide in 'hidden') {
     inputs <- switch(population,ordinary=c(10),missing=c(NA_real_))
     calls <- list()
...
jsonlite::write_json(list(reference='ggplot2 4.0.3',cases=cases),'fixtures/parity/ggplot2/scale-pipeline-singletons.json',auto_unbox=TRUE,pretty=TRUE,digits=15,null='null',na='null')
cat('captured',length(cases),'constructor OOB/rescaler cases\n')
