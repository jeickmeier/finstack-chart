...
for(family in c('cardinality','center'))for(composed in c(FALSE,TRUE)) {
 ...
 for(population in c('ordinary','nullable','empty'))for(layers in c('one','two'))for(major_mode in c('auto','pair'))for(mode in c('explicit','domain','majors','null','mixed_unnamed')) {
  x <- switch(population,ordinary=c(1,2,4),nullable=c(1,NA_real_,4),empty=numeric());y <- if(population=='empty')numeric()else c(2,10)
  calls <- list()
  fn <- function(limits,breaks){calls[[length(calls)+1L]] <<- list(domain=encode(limits),major=encode(breaks));switch(mode,domain=limits,majors=breaks,null=NULL,mixed_unnamed=c(limits[2],mean(limits),limits[1],limits[1],NA,Inf,-Inf))}
  result <- if(!is.null(tr$error))tr else tryCatch(suppressWarnings({
   p <- ggplot(data.frame(x=x),aes(x,1))+geom_point()+scale_x_continuous(transform=tr,breaks=if(major_mode=='pair')c(2,5)else waiver(),minor_breaks=if(mode=='explicit')c(2,5)else fn)
  ...
  cases[[length(cases)+1L]] <- list(configuration=length(configs)-1L,route='position',population=population,layers=layers,limits='auto',count=NULL,major_mode=major_mode,break_values=if(major_mode=='pair')encode(c(2,5))else NULL,minor_mode=mode,inputs=encode(x),second=if(layers=='two')encode(y)else NULL,minor_calls=calls,result=result)
 }
}
jsonlite::write_json(list(reference='ggplot2 4.0.3 / scales 1.4.0 / R 4.6.1',configurations=configs,cases=cases),'fixtures/parity/ggplot2/vector-position-minors.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat(length(cases),'vector position minor builds captured\n')
