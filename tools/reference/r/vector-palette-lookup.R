   ...
   for(limits in c('none','full'))
    for(mode in c('full','short','empty','null','named','missing'))
     ...
     result <- tryCatch(suppressWarnings({
      args <- list(aesthetics=channel,palette=palette,limits=if(limits=='full')if(family=='discrete')c('a','b','c','d')else c(1,10)else NULL,guide=if(guide_mode=='hidden')'none'else'legend',breaks=if(guide_mode=='first')1 else waiver())
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/vector-palette-lookup.json',auto_unbox=TRUE,pretty=TRUE,digits=NA,null='null')
