   ...
   for (population in c('ordinary','missing','empty'))
    for (mode in c('default','reverse','index','short','empty','null')) {
     ...
     result <- tryCatch(suppressWarnings({
      d <- data.frame(v=inputs,g=groups,i=seq_along(inputs),f=factor(facets,levels=if(panels=='retained') c('A','B','C') else c('A','B')))
...
jsonlite::write_json(list(reference='ggplot2 4.0.3',cases=cases),'fixtures/parity/ggplot2/positional-pipeline-binned-facets.json',auto_unbox=TRUE,pretty=TRUE,digits=15,null='null',na='null')
dev.off()
