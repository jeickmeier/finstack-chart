...
library(ggplot2)
# Reference grob measurement must not write the repository default Rplots.pdf.
pdf(file=tempfile("palette-reference-",fileext=".pdf"))
...
cases <- list()
for(family in 'discrete')
 for(channel in 'colour')
  for(pop in c('ordinary','missing','all_missing'))
   for(limits in 'none')
    for(mode in c('full','short','empty','null','named','missing'))
     for(na_mode in c('NA','transparent','green')) {
     ...
     result <- tryCatch(suppressWarnings({
      args <- list(aesthetics=channel,palette=palette,limits=if(limits=='full')if(family=='discrete')c('a','b','c','d')else c(1,10)else NULL,guide='legend',na.value=switch(na_mode,`NA`=NA_character_,transparent='#00000000',green='#00ff00'))
      ...
      grob <- ggplotGrob(built);panel <- grob$grobs[[which(grob$layout$name=='panel')]]
      point_count <- sum(vapply(panel$children,function(g)if(inherits(g,'points'))length(g$x)else 0L,integer(1)))
      list(mapped=encode(built$data[[1]][[channel]]),keys=unname(keys),point_count=point_count)
     }),error=function(e)list(error=conditionMessage(e)))
     cases[[length(cases)+1L]] <- list(family=family,channel=channel,population=pop,limits=limits,mode=mode,na_mode=na_mode,inputs=encode(values),calls=calls,result=result)
    }
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/discrete-palette-missing-paint.json',auto_unbox=TRUE,pretty=TRUE,digits=NA,null='null')
