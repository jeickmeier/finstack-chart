...
cases<-list()
for(context in c('points','summary'))for(transform in c('identity','sqrt','reverse'))for(population in names(sets))for(policy in c('fixed','free'))for(control in c('identity','reverse','fixed','lower_zero','missing_lower','empty','single'))for(oob in c('censor','squish','keep')){
 ...
 result<-tryCatch(suppressWarnings({
  ...
  args<-list(transform=transform,limits=fun,oob=switch(oob,censor=scales::oob_censor,squish=scales::oob_squish,keep=scales::oob_keep))
 ...
 cases[[length(cases)+1]]<-list(context=context,transform=transform,population=population,policy=policy,control=control,oob=oob,inputs=encode(values),groups=as.list(as.character(groups)),levels=as.list(levels),seen=seen,result=result)
}
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/positional-facet-functions.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
