...
cases=list()
for(nbin in c(0,1,2.5,300)) for(display in 'raster') for(reverse in FALSE) for(direction in 'vertical') for(palette in c('ordinary','asymmetric','discontinuous')) {
 warnings=character()
 result=tryCatch(withCallingHandlers({
   p=ggplot(data.frame(x=c(1,2,3,4),v=c(-2,0,3,8)),aes(x,1,colour=v))+geom_point()+
      scale_colour_gradientn(colours=if(palette=='ordinary')c('#000000','#ffffff')else if(palette=='asymmetric')c('#0000ff','#ffffff','#ff0000')else c('#ff0000','#ff0000','#0000ff','#0000ff'),
       ...
       limits=c(3,3),breaks=3,guide=guide_colourbar(nbin=nbin,display=display,reverse=reverse,direction=direction))
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/colorbar-constant.json',pretty=TRUE,auto_unbox=TRUE,digits=17,null='null',na='null')
