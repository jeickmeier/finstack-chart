  ...
  for(pop in c('ordinary','constant','missing','all_missing','empty'))
   for(limits in c('none','full'))
    for(mode in c('full','short','empty','null','named','missing'))
     for(guide_mode in c('hidden','first')) {
     ...
     result <- tryCatch(suppressWarnings({
      args <- list(aesthetics=channel,palette=palette,limits=if(limits=='full')if(family=='discrete')c('a','b','c','d')else c(1,10)else NULL,guide=if(guide_mode=='hidden')'none'else'legend',breaks=if(guide_mode=='first')'a'else waiver())
     ...
     cases[[length(cases)+1L]] <- list(family=family,channel=channel,population=pop,limits=limits,mode=mode,guide_mode=guide_mode,inputs=encode(values),calls=calls,result=result)
    }
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/discrete-palette-lookup.json',auto_unbox=TRUE,pretty=TRUE,digits=NA,null='null')
