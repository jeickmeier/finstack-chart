...
cases<-list()
for(kind in c('date','datetime'))for(context in c('points','summary'))for(population in names(sets))for(control in c('identity','reverse','fixed','lower_zero','missing_lower','empty','single'))for(oob in c('censor','squish','keep')){
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/positional-temporal-functions.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat('PASS',length(cases),'positional temporal callback builds\n')
