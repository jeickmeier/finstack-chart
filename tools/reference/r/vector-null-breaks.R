...
for(family in c('cardinality','center'))for(composed in c(FALSE,TRUE)) {
 ...
 for(kind in c('continuous','binned'))for(route in c('size','paint'))for(population in c('ordinary','nullable','empty'))for(guide in c('legend','none'))for(layers in c('one','two'))for(limit_mode in c('none','reverse'))for(break_mode in c('null')) {
  ...
  break_fn <- function(x){break_calls[[length(break_calls)+1L]] <<- list(values=encode(x),names=as.list(names(x)),is_null=is.null(x));if(break_mode=='null')NULL else if(break_mode=='domain')x else setNames(c(x[2],mean(x),x[1],x[1],NA,Inf,-Inf),c('last','middle','first','again','missing','positive','negative'))}
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / scales 1.4.0 / R 4.6.1',configurations=configs,cases=cases),'fixtures/parity/ggplot2/vector-null-breaks.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
