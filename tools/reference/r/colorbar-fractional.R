...
cases=list()
for(nbin in c(.1,.5,.9,1.1,1e-30)) for(display in 'raster') for(reverse in FALSE) for(direction in 'vertical') for(palette in c('ordinary','asymmetric','discontinuous')) {
 warnings=character()
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/colorbar-fractional.json',pretty=TRUE,auto_unbox=TRUE,digits=17,null='null',na='null')
