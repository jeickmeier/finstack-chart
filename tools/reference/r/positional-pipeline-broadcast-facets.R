# FIX-GG04: source rows broadcast to fixed/free facet populations.
   ...
   for (population in c('ordinary','missing','empty'))
    for (mode in c('default','reverse','index','short','empty','null')) {
     ...
     result <- tryCatch(suppressWarnings({
      ...
      skeleton <- data.frame(f=factor(c('A','B','C'),levels=c('A','B','C')))
...
jsonlite::write_json(list(reference='ggplot2 4.0.3',cases=cases),'fixtures/parity/ggplot2/positional-pipeline-broadcast-facets.json',auto_unbox=TRUE,pretty=TRUE,digits=15,null='null',na='null')
dev.off()
