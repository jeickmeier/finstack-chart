# FIX-GG04: exact Date/POSIXct inputs to color-scale label functions.
    ...
    for(control in c('automatic','explicit','empty','format'))
     for(mode in c('indexed','missing','short','empty')) {
      ...
      result <- tryCatch(suppressWarnings({
       scale <- do.call(if(kind=='date')scale_colour_date else scale_colour_datetime,args)
       b <- ggplot_build(ggplot(data.frame(x=seq_along(values),v=values),aes(x,1,colour=v))+geom_point()+scale)
       keys <- lapply(b$plot$guides$params,function(g)list(values=encode(g$key$.value),labels=encode(g$key$.label)))
       list(keys=unname(keys),limits=encode(b$plot$scales$get_scales('colour')$get_limits()))
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/temporal-label-functions.json',auto_unbox=TRUE,pretty=TRUE,digits=NA,null='null')
