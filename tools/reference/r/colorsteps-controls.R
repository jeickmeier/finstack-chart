...
cases=list()
for(even_steps in c(TRUE,FALSE)) for(show_limits in c(FALSE,TRUE)) for(family in c('continuous','binned')) for(population in c('ordinary','constant','empty')) for(mode in c('automatic','null','empty','one','duplicates','outside','nonfinite','unsorted','limits','endpoint_first')) {
 ...
 breaks=switch(mode,automatic=waiver(),null=NULL,empty=numeric(),one=if(population=='constant')2 else 0,duplicates=c(0,0,3),outside=c(-4,0,3,10),nonfinite=c(NA_real_,Inf,-Inf,0,3),unsorted=c(8,3,0,-2),limits=c(-2,8),endpoint_first=c(8,0,3))
 result=tryCatch(withCallingHandlers({
 args=list(colours=c('#0000ff','#ffffff','#ff0000'),values=c(0,.2,1),limits=limits,breaks=breaks,guide=guide_coloursteps(even.steps=even_steps,show.limits=show_limits))
 ...
 cases[[length(cases)+1L]]=list(even_steps=even_steps,show_limits=show_limits,family=family,population=population,mode=mode,limits=encode(limits),inputs=encode(values),breaks=if(mode=='automatic')'automatic'else if(mode=='null')NULL else encode(breaks),warnings=as.list(warnings),result=result)
}
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/colorsteps-controls.json',pretty=TRUE,auto_unbox=TRUE,digits=17,null='null',na='null')
invisible(dev.off())
