# FIX-GG04: factor retention and missing-category discrete label callbacks through actual guide preparation.
...
for (channel in c('size', 'alpha', 'linewidth', 'shape', 'linetype'))
 for (population in c('ordinary', 'nullable', 'all_missing', 'empty'))
  for (limit_mode in c('trained', 'explicit'))
   for (drop in c(FALSE, TRUE)) for (translate in c(FALSE, TRUE))
   for (break_mode in c('auto', 'named'))
    for (mode in c('indexed', 'short')) {
     values <- switch(population, ordinary = c('a', 'b', 'c'),
                      nullable = c('a', 'b', NA), all_missing = c(NA_character_, NA_character_),
                      empty = character())
     factor_levels <- c('c', 'b', 'a', 'd')
     values <- factor(values, levels = factor_levels)
     calls <- list()
     ...
     result <- tryCatch(suppressWarnings({
      ...
      scale <- constructor(limits = if (limit_mode == 'explicit') c('c', 'b', 'a', NA) else NULL,
       ...
       labels = label_function, drop = drop, na.translate = translate)
     ...
     cases[[length(cases) + 1L]] <- list(channel = channel, population = population,
      ...
      inputs = encode(as.character(values)), levels = as.list(factor_levels),
      drop = drop, na_translate = translate, calls = calls, result = result)
    }
invisible(dev.off())
jsonlite::write_json(list(reference = 'ggplot2 4.0.3 / R 4.6.1', cases = cases),
 'fixtures/parity/ggplot2/factor-discrete-label-functions.json',
