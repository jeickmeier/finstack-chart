# FIX-GG04: actual Date/datetime positional callbacks before and after statistics.
...
sets<-list(thirds=c(0,1,3),sevenths=c(0,0,0,0,0,0,1))
cases<-list()
for(kind in c('date','datetime'))for(context in c('points','summary'))for(population in names(sets))for(control in c('identity','lower_zero','fixed'))for(oob in c('censor','squish','keep')){
...
jsonlite::write_json(list(reference='ggplot2 4.0.3 / R 4.6.1',cases=cases),'fixtures/parity/ggplot2/positional-temporal-fractional-statistics.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat('PASS',length(cases),'fractional temporal statistic builds\n')
