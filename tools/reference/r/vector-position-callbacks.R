...
for(family in c('cardinality','center'))for(composed in c(FALSE,TRUE)) {
 ...
 for(population in c('ordinary','nullable','empty'))for(layers in c('one','two'))for(mode in c('null','domain','mixed')) {
  x <- switch(population,ordinary=c(1,2,4),nullable=c(1,NA_real_,4),empty=numeric());y <- if(population=='empty')numeric()else c(2,10)
  calls <- list()
  fn <- function(x){calls[[length(calls)+1L]] <<- list(values=encode(x),is_null=is.null(x));if(mode=='null')NULL else if(mode=='domain')x else setNames(c(x[2],mean(x),x[1],x[1],NA,Inf,-Inf),c('last','middle','first','again','missing','positive','negative'))}
  result <- if(!is.null(tr$error))tr else tryCatch(suppressWarnings({
   p <- ggplot(data.frame(x=x),aes(x,1))+geom_point()+scale_x_continuous(transform=tr,breaks=fn)
  ...
  cases[[length(cases)+1L]] <- list(configuration=length(configs)-1L,route='position',population=population,layers=layers,limits='auto',count=NULL,break_mode=mode,inputs=encode(x),second=if(layers=='two')encode(y)else NULL,break_calls=calls,result=result)
 }
}
jsonlite::write_json(list(reference='ggplot2 4.0.3 / scales 1.4.0 / R 4.6.1',configurations=configs,cases=cases),'fixtures/parity/ggplot2/vector-position-callbacks.json',auto_unbox=TRUE,pretty=TRUE,digits=17,null='null',na='null')
cat(length(cases),'vector position callback builds captured\n')
