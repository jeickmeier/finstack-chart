...
for(family in 'binned')
 for(limit_mode in c('full','automatic','callback'))
 for(channel in c('colour','size','alpha'))
  for(population in c('ordinary','missing','empty'))
   for(mode in c('default','oob_reverse','rescale_index','rescale_short'))
    for(guide in c('none','legend','bins','coloursteps','default')) {
     ...
     result <- tryCatch(suppressWarnings({
      arguments<-list(aesthetics=channel,palette=palette,limits=switch(limit_mode,full=c(1,10),automatic=NULL,callback=function(x)x),oob=oob,rescaler=rescaler)
      if(guide!='default')arguments$guide<-guide
      scale<-do.call(if(family=='continuous')continuous_scale else binned_scale,arguments)
     ...
     cases[[length(cases)+1L]]<-list(family=family,channel=channel,population=population,mode=mode,guide_kind=guide,limit_mode=limit_mode,inputs=encode(inputs),calls=calls,result=result)
    }
jsonlite::write_json(list(reference='ggplot2 4.0.3',cases=cases),'fixtures/parity/ggplot2/binned-guide-selection.json',auto_unbox=TRUE,pretty=TRUE,digits=15,null='null',na='null')
