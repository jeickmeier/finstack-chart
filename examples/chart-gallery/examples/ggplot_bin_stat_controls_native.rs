//! FIX-GG06 composed guide native proof.
#[path = "../../common/ggplot_bin_stat_controls.rs"]
mod fixtures;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plots = [0, 2, 3, 4, 40, 80]
        .into_iter()
        .map(|mode| {
            assert!(fixtures::selected(&cases[mode]));
            Ok((
                fixtures::name(&cases[mode]),
                fixtures::author(&cases[mode])?,
            ))
        })
        ...
        cx.open_window(
            WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some("GGplot Bin Statistics Proof".into()),
                    ..Default::default()
                }),
                ..Default::default()
            },
