...
#[path = "../../common/ggplot_furniture_controls.rs"]
mod fixtures;
                ...
                div()
                    ...
                    .children(self.charts.iter().map(|(name, chart)| {
                        div()
                            ...
                            .child(div().w(px(360.)).h(px(320.)).child(chart.clone()))
                    })),
            )
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let plots = (0..fixtures::CASES)
        .map(|mode| Ok((format!("Furniture {mode}"), fixtures::author(mode)?)))
        ...
        cx.open_window(
            WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some("GGplot Furniture Controls".into()),
                    ..Default::default()
                }),
                ..Default::default()
            },
