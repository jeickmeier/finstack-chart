//! FIX-GG13 native recipe and target proof.
#[path = "../../common/ggplot_coordinate_controls.rs"]
mod fixtures;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let modes = std::env::var("GG13_MODES")
        .ok()
        .map(|v| {
            v.split(',')
                .map(str::parse::<usize>)
    ...
    let plots = modes
        .into_iter()
        .map(|mode| Ok((format!("Coordinate {mode}"), fixtures::author(mode)?)))
        ...
        cx.open_window(
            WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some("GGplot Coordinate Controls".into()),
                    ..Default::default()
                }),
                ..Default::default()
            },
