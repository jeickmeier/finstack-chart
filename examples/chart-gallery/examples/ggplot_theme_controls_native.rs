//! FIX-GG14 native theme controls proof.
#[path = "../../common/ggplot_theme_controls.rs"]
mod fixtures;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let modes = if let Some(mode) = std::env::args().nth(1) {
        ...
        if mode >= fixtures::CASES {
            return Err("Theme mode must be 0..15".into());
        }
        vec![mode]
    } else {
        vec![0, 3, 4, 6, 7, 10]
    ...
    let plots = modes
        .into_iter()
        .map(|mode| Ok((format!("Theme {mode}"), fixtures::author(mode)?)))
        ...
        cx.open_window(
            WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some("GGplot Theme Controls".into()),
                    ..Default::default()
                }),
                ..Default::default()
            },
