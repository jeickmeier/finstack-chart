//! FIX-GG09 native recipe and target proof.
#[path = "../../common/ggplot_distribution_geometries.rs"]
mod fixtures;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let plots = [1, 6, 8, 9, 11, 14]
        .into_iter()
        .map(|mode| Ok((format!("Recipe {mode}"), fixtures::author(mode)?)))
