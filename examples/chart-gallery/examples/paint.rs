//! CLR-04: actual native rendering of retained floating paint inputs.
#[path = "../../common/paint_fixtures.rs"]
mod fixtures;
                ...
                div()
                    ...
                    .children(self.charts.iter().map(|(name, chart)| {
                        div()
                            ...
                            .child(div().w(px(440.)).h(px(340.)).child(chart.clone()))
                    })),
            )
    ...
    gpui_platform::application().run(move |cx| {
        ...
        let inputs = plots
            ...
            .map(|(name, plot)| {
                (
                    (*name).to_owned(),
                    ChartInput::from_plot(plot, font.clone()).expect("paint input"),
                )
            })
            .collect::<Vec<_>>();
        cx.open_window(
            WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some("Color Paint Proof".into()),
                    ..Default::default()
                }),
                ..Default::default()
            },
